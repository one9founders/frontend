...
export async function getReviewsByToolId(toolId: number) {
  ...
  try {
    const response = await fetch(`${API_URL}/api/reviews/?tool_id=${toolId}`, {
...
export async function getToolUsageCount(toolId: number) {
  const API_URL = process.env.NEXT_PUBLIC_API_URL || 'https://api.one9founders.com';
  try {
    const response = await fetch(`${API_URL}/api/tools/${toolId}/usage-count/`, {
...
export async function getAllToolSlugs() {
  const API_URL = process.env.NEXT_PUBLIC_API_URL || 'https://api.one9founders.com';
  try {
    const response = await fetch(`${API_URL}/api/tools/?page_size=1000`, {
