...
export default function RagToolDetailClient({ tool }: RagToolDetailClientProps) {
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <Link href="/rag-vector-dbs" className="inline-flex items-center gap-1 text-sm text-[var(--gray-500)] hover:text-[var(--paper)] transition-colors mb-8">
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
        Back to directory
      </Link>

      <div className="tool-animate-in flex items-start gap-4 md:gap-6 mb-6">
        <ToolLogo logoUrl={tool.logo_url} name={tool.name} size="lg" containerClassName="shadow-[0_0_0_1px_var(--line)]" />
        <div className="flex-1 min-w-0">
          <p className="tool-section-label mb-2">{catConfig.label}</p>
          <div className="flex items-center gap-3 flex-wrap mb-3">
            <h1 className="font-display text-3xl md:text-5xl font-bold text-[var(--paper)] leading-[0.95] tracking-tight">{tool.name}</h1>
            {tool.overall_rating > 0 && (
              <span className="tool-chip tool-chip-accent tabular-nums">
                {tool.overall_rating.toFixed(1)} rating
              </span>
            )}
            <CategoryPill label={statConfig.label} variant={statConfig.variant} size="sm" />
          </div>
          <p className="text-[var(--gray-400)] text-base md:text-lg mb-4 max-w-2xl leading-relaxed">{tool.description}</p>
      ...
      <div className="tool-animate-in tool-animate-in-delay-1 flex flex-wrap gap-3 mb-10">
        {tool.website_url && (
        ...
        {tool.docs_url && (
          <a href={tool.docs_url} target="_blank" rel="noopener noreferrer" className="px-5 py-2.5 rounded-lg font-medium text-sm text-[var(--gray-300)] border border-[var(--line)] hover:border-copper/40 transition-colors inline-flex items-center gap-2">
            Documentation
          </a>
        )}
        {tool.github_url && (
          <a href={tool.github_url} target="_blank" rel="noopener noreferrer" className="px-5 py-2.5 rounded-lg font-medium text-sm text-[var(--gray-300)] border border-[var(--line)] hover:border-copper/40 transition-colors inline-flex items-center gap-2">
            GitHub
          </a>
        )}
      </div>
      ...
      <section className="tool-section">
        ...
        <h2 className="font-display text-2xl md:text-3xl font-bold text-[var(--paper)] mb-5 leading-tight">What it is</h2>
      ...
      {tool.rating_scores && Object.keys(tool.rating_scores).length > 0 && (
        <section className="tool-section">
          <p className="tool-section-label">Scores</p>
          <h2 className="font-display text-2xl md:text-3xl font-bold text-[var(--paper)] mb-5 leading-tight">Rating breakdown</h2>
          <div className="tool-meta-panel">
            <div className="flex items-center gap-4 mb-6">
              <div className="font-display text-4xl font-bold text-[var(--paper)]">{tool.overall_rating.toFixed(1)}</div>
            ...
            <div className="space-y-3">
              {Object.entries(tool.rating_scores).map(([key, value]) => (
                <div key={key} className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-copper-bright font-medium w-8 text-right tabular-nums">{value.toFixed(1)}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {tool.specs && Object.keys(tool.specs).length > 0 && (
        <section className="tool-section">
          <p className="tool-section-label">Specs</p>
          <h2 className="font-display text-2xl md:text-3xl font-bold text-[var(--paper)] mb-5 leading-tight">Technical specs</h2>
          <div className="tool-meta-panel">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {tool.specs.index_types && tool.specs.index_types.length > 0 && (
                <div>
                  ...
                  <p className="text-[var(--paper)] text-sm mt-1">{tool.specs.index_types.join(', ')}</p>
                </div>
              )}
              {tool.specs.distance_metrics && tool.specs.distance_metrics.length > 0 && (
                <div>
                  ...
                  <p className="text-[var(--paper)] text-sm mt-1">{tool.specs.distance_metrics.join(', ')}</p>
                </div>
              )}
              {tool.specs.max_dimensions != null && (
                <div>
                  <span className="text-sm text-[var(--gray-500)]">Max Dimensions</span>
                  <p className="text-[var(--paper)] text-sm mt-1">{tool.specs.max_dimensions.toLocaleString()}</p>
                </div>
              )}
              {tool.specs.hybrid_search != null && (
                <div>
                  <span className="text-sm text-[var(--gray-500)]">Hybrid Search</span>
                  <p className="text-[var(--paper)] text-sm mt-1">{tool.specs.hybrid_search ? 'Yes' : 'No'}</p>
                </div>
              )}
            </div>
              ...
              <div className="mt-4 pt-4 border-t border-[var(--line)]">
                ...
                <div className="flex flex-wrap gap-2">
                  {tool.sdk_languages.map((lang) => (
                    <span key={lang} className="tool-chip">{lang}</span>
                  ))}
                </div>
              </div>
            )}
              ...
              <div className="mt-4 pt-4 border-t border-[var(--line)]">
                ...
                <div className="flex flex-wrap gap-2">
                  {tool.integrations.map((int_) => (
                    <span key={int_} className="tool-chip tool-chip-accent">{int_}</span>
                  ))}
                </div>
              </div>
            )}
              ...
              <div className="mt-4 pt-4 border-t border-[var(--line)]">
                ...
                <div className="flex flex-wrap gap-2">
                  {tool.security_certs.map((cert) => (
                    <span key={cert} className="tool-chip tool-chip-ok">{cert}</span>
                  ))}
                </div>
              </div>
            )}
      ...
      <section className="tool-section">
        <p className="tool-section-label">Pricing</p>
        <h2 className="font-display text-2xl md:text-3xl font-bold text-[var(--paper)] mb-5 leading-tight">Pricing</h2>
        <div className="tool-meta-panel">
      ...
      {(tool.github_stars > 0 || (tool.github_snapshots && tool.github_snapshots.length > 0)) && (
        <section className="tool-section">
          <p className="tool-section-label">Open source</p>
          <h2 className="font-display text-2xl md:text-3xl font-bold text-[var(--paper)] mb-5 leading-tight">GitHub activity</h2>
          <div className="tool-meta-panel">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              <div>
                <span className="text-sm text-[var(--gray-500)]">Stars</span>
                <p className="text-xl font-bold text-[var(--paper)]">{formatStars(tool.github_stars)}</p>
              </div>
              <div>
                <span className="text-sm text-[var(--gray-500)]">Forks</span>
                <p className="text-xl font-bold text-[var(--paper)]">{formatStars(tool.github_forks)}</p>
              </div>
              <div>
                <span className="text-sm text-[var(--gray-500)]">Last Commit</span>
                <p className="text-sm font-medium text-[var(--paper)]">{formatDate(tool.last_commit_at)}</p>
              </div>
              <div>
                <span className="text-sm text-[var(--gray-500)]">Latest Release</span>
                <p className="text-sm font-medium text-[var(--paper)]">{tool.latest_release || 'N/A'}</p>
              </div>
            </div>
      ...
      {tool.similar_tools && tool.similar_tools.length > 0 && (
        <section className="tool-section">
          ...
          <h2 className="font-display text-2xl md:text-3xl font-bold text-[var(--paper)] mb-5 leading-tight">Similar solutions</h2>
